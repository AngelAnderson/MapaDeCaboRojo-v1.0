...
const ContactModal: React.FC<ContactModalProps> = ({ isOpen, onClose, onSuggest, onAbout }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[2000] flex items-end sm:items-center justify-center p-4 animate-fade-in">
      ...
      <div className="bg-paper w-full max-w-sm rounded-[32px] shadow-2xl overflow-hidden relative z-10 animate-slide-up border border-white/20 dark:border-line transition-colors">
        
        {/* Header */}
        <div className="bg-canvas/50 p-6 pb-4 text-center border-b border-line">
          <div className="w-14 h-14 bg-paper rounded-full mx-auto flex items-center justify-center shadow-lg border border-line mb-3 text-2xl animate-float">
            🤝
          </div>
          <h2 className="text-xl font-black text-ink tracking-tight">{t('contact_title')}</h2>
          <p className="text-ink-muted text-xs font-medium mt-1">{t('contact_subtitle')}</p>
        </div>
        ...
        <button onClick={onAbout} className="w-full p-4 bg-paper-2 text-ink-muted text-xs font-bold hover:text-brand-600 dark:hover:text-brand-400 transition-colors border-t border-line">
            {t('about_title')}
        </button>
      </div>
    </div>
