    ...
    <div className="min-h-screen bg-canvas font-sans transition-colors">
      ...
      <header className="sticky top-0 z-50 bg-paper/90 backdrop-blur-xl border-b border-line px-4 py-4 flex justify-between items-center shadow-sm">
          <div className="flex items-center gap-3">
              ...
              <div>
                  <h1 className="text-lg font-black text-ink leading-none">{t('suggest_title')}</h1>
                  <p className="text-xs text-ink-muted font-medium">{t('suggest_subtitle')}</p>
              </div>
          </div>
          <button onClick={handleGoHome} className="w-10 h-10 rounded-full bg-paper-2 text-ink-muted hover:text-ink dark:hover:text-white flex items-center justify-center transition-colors">
      ...
      <main className="max-w-2xl mx-auto p-6 pb-32">
        ...
        {step === 1 && (
            <div className="space-y-6 animate-slide-up">
              ...
              <div>
                  ...
                  <p className="text-[10px] text-ink-muted mt-1 ml-1">Ayuda a encontrar la ubicación exacta.</p>
              </div>
        ...
        {step === 2 && (
            <div className="space-y-6 animate-slide-up">
              ...
              <div className="bg-paper p-5 rounded-xl border border-line-strong space-y-4">
                <div className="flex justify-between items-center">
                    <span className="font-bold text-ink">{t('suggest_parking')}</span>
                    <input type="checkbox" checked={parking === ParkingStatus.FREE} onChange={e => setParking(e.target.checked ? ParkingStatus.FREE : ParkingStatus.PAID)} className="w-6 h-6 accent-brand-600 rounded cursor-pointer" />
                </div>
                <div className="flex justify-between items-center">
                    <span className="font-bold text-ink">{t('suggest_restroom')}</span>
                    <input type="checkbox" checked={hasRestroom} onChange={e => setHasRestroom(e.target.checked)} className="w-6 h-6 accent-brand-600 rounded cursor-pointer" />
                </div>
                <div className="flex justify-between items-center">
                    <span className="font-bold text-ink">{t('suggest_pet')}</span>
                    <input type="checkbox" checked={isPetFriendly} onChange={e => setIsPetFriendly(e.target.checked)} className="w-6 h-6 accent-brand-600 rounded cursor-pointer" />
                </div>
              </div>

              <div className="flex gap-3 pt-4">
                <button onClick={() => setStep(1)} className="px-6 py-4 rounded-xl text-ink-muted font-bold bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-paper-2 transition-colors">{t('back')}</button>
        ...
        {step === 3 && (
            <div className="text-center py-12 animate-fade-in">
                ...
                <h3 className="text-3xl font-black text-ink mb-4">{t('suggest_success_title')}</h3>
                <p className="text-ink-soft text-lg leading-relaxed max-w-md mx-auto">{t('suggest_success_msg_admin_review')}</p>
