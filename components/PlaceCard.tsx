...
const PlaceCard: React.FC<PlaceCardProps> = ({ place, onClose, onNavigate, onAskAi, onSuggestEdit }) => {
  const { t } = useLanguage();
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-800 rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.3)] dark:shadow-[0_-10px_40px_rgba(0,0,0,0.6)] z-[2000] animate-slide-up max-h-[90vh] overflow-y-auto flex flex-col transition-colors duration-300" role="dialog" aria-modal="true" aria-labelledby="place-name">
      ...
      <div className="relative w-full h-72 shrink-0 group">
        ...
        <div className="absolute bottom-0 left-0 p-6 text-white w-full">
          ...
          <h2 id="place-name" className="text-3xl font-black leading-tight shadow-black drop-shadow-md">{place.name}</h2>
      ...
      <div className="p-6 space-y-6 bg-white dark:bg-slate-800 -mt-4 rounded-t-3xl relative z-10 flex-1 transition-colors duration-300">
        ...
        {(place.address || place.phone || place.gmapsUrl) && (
            <div className="bg-slate-50 dark:bg-slate-700/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-600 space-y-3 transition-colors">
                {place.address && (
